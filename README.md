...
Bridgething app catalog for Block Drop.

Webapps for the Spotify Car Thing running [bridgething](https://bridgething.com).

## First run

1. Push this repo to `https://github.com/dharmapoudel/bridgething-block-drop-source`.
2. In **Settings > Pages**, set the source to **Deploy from a branch**, branch `gh-pages`, folder `/ (root)`.

The catalog is published to `https://dharmapoudel.github.io/bridgething-block-drop-source/catalog.v1.json`, which can be submitted to <bridgething.com/apps>
...
With more than one app in `apps/` your commands must specify which one: `bun run dev radio-atlas`.
...
```sh
bun run bump radio-atlas patch -m "Fix the wind direction arrow"
git commit -am "radio-atlas: fix the wind direction arrow" && git push
```
